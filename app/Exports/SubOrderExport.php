...
public function __construct($type,$str,$daterange,$vendor,$brand,$category)
{
    $this->type = $type;
	$this->str = $str;
		 ...
		 $Orders=OrdersDetail::select(
			...
			if($category_id!='All' && $category_id!=''){
		  	$Orders =$Orders->where('product_categories.cat_id',$category_id);
				...
				$Orders=$Orders->where('order_details.order_status',$type)->groupBy('order_details.id')->orderBy('order_details.id','desc')->get();
