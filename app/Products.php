...
class Products extends Model
{
	...
	public static function getProductMaterial($id){
		$material = Materials::select('name')->where('id',$id)->first();
   ...
   public static function brandDescription($brand,$type){
	...
	public static function ofr_per($price,$spcl_price){
		...
		$offer_percentage=round(($newPrice*100)/$default_price);
	...
	public static function getFirstattrId($table,$prd_id){
		...
		public static function getcolorNameAndCode($table='Colors',$id=0){
			...
			public static function getcolorCode($table='Colors',$id=0){
			    $code='#ffffff';
		    	$name=Colors::select('name','color_code','color_image')->where('id',$id)->first();
			if($name){
   ...
   return $html;
	...
	public static function productRatingCounterNumber($id){
		return $final_rating=self::productRatings($id);
	}
...
public static function offerPercentage($price,$spcl_price){
		...
		//   $offer_percentage=ceil(($cutPrice*100)/$default_price);
		  $offer_percentage=round(($cutPrice*100)/$default_price); 
		return $offer_percentage;  
		} else{
		   return 0; 
		}
...
public static function getProductsAttributesWsize($attr_id,$prd_id){
	...
	function getProductsVendor(){
		$name=Vendor::select('username')->where('id',$this->vendor_id)->first();
...
function updateInfo($arr,$prd_id){
    ...
    // }
			...
			'additional_Information' => $arr['additional_Information'],
            'sku' => $arr['sku'],
            //'gtin' => $arr['gtin'],
	...
	public static function prdImages($id){
		...
		$res=Products::select('id','default_image','zoom_image')->where('id', '=',$id)
	         	->first();
