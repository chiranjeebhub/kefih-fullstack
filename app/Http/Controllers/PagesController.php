   ...
   public function about()
    ...
    public function couponList(){
        $coupons=Coupon::select(
           ...
           'coupon_details.coupon_used' ,
           'coupon_details.status'
            )
        ->join('coupon_details','coupons.id','coupon_details.coupon_id')
        ->where('coupon_details.coupon_used','=','0')
        ->whereIn('coupons.coupon_type',['0','1'])
        // ->orwhere('coupons.coupon_type','1')
        // ->where('coupons.max_discount','>', 0) 
        // ->whereNotNull('coupons.max_discount')
        ->where('coupon_details.status',1)
        ->orderBy('coupons.created_at', 'DESC')
        // ->groupBy('coupon_details.coupon_code')
        ->paginate(100);  
   
        return view('fronted.mod_pages.coponlist',compact('coupons'));
    ...
    //$page_data = DB::table('tbl_faq')->where('fld_faq_status',1)->orderBy('id','asc')->paginate(100);

return view('fronted.mod_pages.faq',array('page_data'=>$page_data,'category_id'=>0));
