...
class CategoryController extends Controller
{
	...
	public function category_export(Request $request)
    {
		...
		return Excel::download(new categoryExport($str,$daterange), 'category'.date('d-m-Y H:i:s').'.csv');	
    }
    public function lists(Request $request)
    {
		$parameters=$request->str;	
		$status=$request->status;	
		$export=URL::to('admin/category_export');
		...
		return view('admin.categories.category.list',['Categories'=>$Categories,'page_details'=>$page_details,'status'=>$status,'parameters'=>$parameters]);
    }
	public function categorywisesizechart(Request $request,$categoryid)
	{
		$id=base64_decode($categoryid);
		$page_details=array(
			"Title"=>"Size chart List",
			"Box_Title"=>"Size chart List"
			  
		  );
		$Sizechart= Sizechart::where('category_id',$id);
			
		$Sizechartdetails=$Sizechart->orderBy('id', 'DESC')->paginate(100);
        
		return view('admin.categories.category.sizechartlist',['Sizechartdetails'=>$Sizechartdetails,'page_details'=>$page_details]);
	}	
 ...
 if ($request->hasFile('size_chart')) {
		...
		$Category->delivery_charge = $input['delivery_charge'];
       ...
       "Form_data"=>array(
         ...
         "Form_field"=>array(
		   ...
		   "delivery_charge"=>array(
			...
			'classes'=>'form-control',
			'placeholder'=>'Delivery Charge',
			'value'=>$Category_detail->delivery_charge,
			'disabled'=>''
	  	 ),
		   "select_field"=>array(
              'label'=>'Select Parent',
         ...
         $request->validate([
            ...
            // 'commission_rate' => 'numeric|min:1',
			 ...
			 $Category->description = $input['description'];
			  $Category->cancel_description = $input['cancel_description'];
			  $Category->return_description = $input['return_description'];
 		$Category->commission_rate = !empty($input['commission_rate']) ? $input['commission_rate'] : 0;
			$Category->featured = $input['featured_category'];
				$Category->cat_shows_in_nav = $input['cat_shown_in_nav'];
					$Category->cat_shows_in_mobile = $input['cat_shows_in_mobile'];
						$Category->cat_shows_in_mobile_cat = $input['cat_shows_in_mobile_cat'];
