...
"fld_assign_type_id"=>$input['category_field']
    ...
    "fld_assign_type_id"=>$input['product_field']
		...
		case 4:
			$request->validate([
			'seller_field' => 'required',
			]
					);
						$data=array(
			"fld_coupon_id"=>$c_id,
			"fld_coupon_assign_type"=>$input['for_category_or_brand_or_product'],
			"fld_assign_type_id"=>$input['seller_field']
						);
				break;
 }
      ...
      return Redirect::back();
          ...
          $brands = Brands::select('id','name')->where('isdeleted', 0)->get();
		      $products = [];
		    $prodId=@$coupon_assign->fld_assign_type_id;
            $selctProducts = Products::select('products.id','products.name','vendor_company_info.city')
            ->join('vendors','products.vendor_id','vendors.id')
            ->join('vendor_company_info','vendor_company_info.vendor_id','vendors.id')
            ->where(function($sql) use ($prodId){
            $sql->where('products.id',$prodId);
            
            })
            ->first();
			
			$SellerData=DB::table('vendors')
							->select('vendors.id','vendor_company_info.name')
							->join('vendor_company_info', 'vendors.id', '=', 'vendor_company_info.vendor_id')
							->where(['vendors.status' => 1,'vendors.isdeleted' => 0])
		...
		"Form_data"=>array(
         ...
         "Form_field"=>array(
			...
			//(object)array("id"=>"2","name"=>"Brand Wise",),
			...
			"seller_field"=>array(
				...
				'name'=>'seller_field',
				'id'=>'seller_field',
				'classes'=>'form-control SellerClass',
				'placeholder'=>'',
				'value'=>$SellerData,
				'disabled'=>'',
				'selected'=>($coupon_assign)?$coupon_assign->fld_assign_type_id:1
 ...
 return view('admin.coupons.assign',['page_details'=>$page_details,'selectProd'=>$selctProducts]);
     }
      ...
      public function getProductName(Request $request)
	{
        ...
        $getDetails = Products::select('products.id','products.name','vendor_company_info.city')
            ->join('vendors','products.vendor_id','vendors.id')
            ->join('vendor_company_info','vendor_company_info.vendor_id','vendors.id')
            ->where(function($sql) use ($term){
	          $sql->where('products.name','like', DB::raw("'$term%'"));
            $sql->where('products.isdeleted', 0);
            $sql->where('products.status',1);
	       ...
	       ->limit(10)->get()->toarray();
			...
			foreach($getDetails as $details){
				$results[] = [
					'id' => $details['id'],
					'text' => $details['name']." (".$details['city'].")",
					'isExist' => 1
				];
			}
			} else {
...
return view('admin.extrafeature.offers.form',['page_details'=>$page_details]);
	 ...
	 public function coupons_export(Request $request)
    {
		$str=$request->str;
		$daterange=$request->daterange;
		return Excel::download(new couponsExport($str,$daterange), 'coupons'.date('d-m-Y H:i:s').'.csv');	
    }
	 
    public function lists(Request $request)
    {
		$type=base64_decode($request->type);	
		$export=URL::to('admin/coupons_export');
		$page_details=array(
       "Title"=>"Coupon List",
       "Box_Title"=>"Coupon(s)",
	   "export"=>$export,
     ...
     if(Auth::guard('vendor')->check()){
          $coupons=Coupon::where('coupon_type',$type)->where('vendor_id', Auth::guard('vendor')->user()->id)->orderBy('created_at', 'DESC')->get();
      
     } else{
         $coupons=Coupon::where('coupon_type',$type)->orderBy('created_at', 'DESC')->get();  
     }
	 ...
	 public function add(Request $request)
    {
	  ...
	  if ($request->isMethod('post')) {
	       ...
	       $rules = [
                    'name' => 'required|max:25',
                    'max_discount' => 'not_in:0',
                    'couponType' => 'required',
                    "coupon_for"=>"required",
                    'short_description' => 'max:255',
                    'discount' => 'required|numeric|min:1|max:100|not_in:0',
                    'couponNumber' => 'required|numeric|min:1|not_in:0',
                    ...
                    if($input['couponType']==0 || $input['couponType']==1 || $input['couponType']==2 || $input['couponType']==3){
                        $rules = [
                                    'uses_per_user' => 'required|not_in:0|min:1',
                                    'number_of_user' => 'required|not_in:0|min:1'
                               
                                ];
                    
                    $message=[
                                    "uses_per_user.required"=>"Per user uses is required",
                                    "number_of_user.required"=>"Number of users can use is required",
                                    "uses_per_user.not_in"=>"Per user uses is should not be 0",
                                    "number_of_user.not_in"=>"Number of users can use is should not be 0",
                                    "uses_per_user.min"=>"Per user uses is required should be minimum 1",
                                    "number_of_user.min"=>"Number of users can use should be minimum 1"
                            ];
                               $request->validate($rules);
                    }
             ...
             $end_date=''; 
		  if($input['issuedate']!=''){
		       $start_date=$input['issuedate']; 
		  }
		  if($input['expire_date_field']!=''){
		      $end_date=$input['expire_date_field'];
		  }
    ...
    if(Auth::guard('vendor')->check()){
        ...
        $Coupon->coupon_type =$input['couponType'];
        $Coupon->description =$input['short_description'];
        $Coupon->max_discount =$input['max_discount'];
        $Coupon->coupon_for =$input['coupon_for'];
      
            $Coupon->number_of_user =$input['number_of_user'];
            $Coupon->uses_per_user =$input['uses_per_user'];
       
        $Coupon->started_date =$start_date;
        $Coupon->end_date =$end_date;
        $Coupon->discount_value = $input['discount'];
        $Coupon->total_coupon = $input['couponNumber'];
         $Coupon->show_in_app = $input['show_in_app'];
      ...
      if($Coupon->save()){
          ...
          for($j=1;$j<=$input['couponNumber'];$j++){
              $characters = date('YmdHis').'0123456789abcdefghijk'; 
                $code = ''; 
                for ($i = 0; $i < 20; $i++) { 
	  ...
	  // "back_url"=>url()->previous(),
	   "back_url"=>route('coupons',base64_encode(0)),
	   ...
	   "Box_Title"=>"Add Coupon",	  
		...
		"Form_data"=>array(
			...
			"coupon_show_inapp__field"=>array(
							'label'=>'Show in App *',
							'type'=>'radio',
							'name'=>'show_in_app',
							'id'=>'show_in_app',
			...
			"number_of_user_field"=>array(
							'label'=>'Number Of users can use*',
							'type'=>'number',
							'name'=>'number_of_user',
							'min'=>'0',
							'id'=>'number_of_user',
			...
			"user_coupon_uses_field"=>array(
							'label'=>'Number Of time users can use*',
							'type'=>'number',
							'name'=>'uses_per_user',
							'min'=>'0',
							'id'=>'uses_per_user',
			...
			"discount_field"=>array(
				...
				"coupon_max_discount_feild"=>array(
							'label'=>'Max Discount',
							'type'=>'number',
							'name'=>'max_discount',
							'min'=>'0',
