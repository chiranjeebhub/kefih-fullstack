...
use URL;
class CategorysizechartController extends Controller
{
    /**
    ...
    public function adminlists(Request $request,$id)
    {
        $parameters=$request->str;	
        $status=$request->status;	
        $id=$id;	
        ...
        $page_details=array(
          ...
          $categoryname=DB::table('vendor_categories')->where('vendor_id',$id)->first(); 
        ...
        return view('admin.categoriessizechart.category.old_list',['Categories'=>$Categories,'page_details'=>$page_details,'status'=>$status,'vendorsid'=>$id]);
	...
	public function adminmulti_delete_cat(Request $request)
    {
			$input=$request->all();
    ...
    public function multi_vendor_delete_cat(Request $request){
	  ...
	  public function adminadd_sizechart(Request $request,$id,$vendor){

      $id=base64_decode($id);

      $vendorid=base64_decode($vendor);

	    $page_details=array(
       "Title"=>"Add Category",
	     "Method"=>"1",
       "Box_Title"=>"Add size chart",
       "Action_route"=>route('adminadd_sizechart', [base64_encode($id),base64_encode($vendorid)]),
       "Form_data"=>array(
	 ...
	 if ($request->isMethod('post')) {
        ...
        if ($request->hasFile('size_chart')) {
          ...
          $uploadsizechart=Sizechart::where('vendor_id',$vendorid)->where('category_id',$id)->get()->count();
    
          if($uploadsizechart>0)
          {
            Sizechart::where('vendor_id',$vendorid)->where('category_id',$id)
                    ->update(['sizechart' => $file_name3]);
          }else{
              $sizechart['vendor_id']=$vendorid;
              $sizechart['category_id']=$id;
              $sizechart['sizechart']=$file_name3;
              Sizechart::create($sizechart);
          }
          return redirect()->route('admincategories',$vendorid);
	 }
    return view('admin.categoriessizechart.category.sizechatform',['page_details'=>$page_details,'categoryid'=>$id,'vendorid'=>$vendorid]);
    }
 ...
 $file_name= FIleUploadingHelper::UploadImage($destinationPath,$size_chart,$file_name);
   ...
   return Redirect::back();
     ...
     public function admindelete_category(Request $request)
    {
            $id=base64_decode($request->id);
    ...
    public function admincat_sts(Request $request)
    {
            $id=base64_decode($request->id);
