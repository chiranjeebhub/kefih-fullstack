    ...
    public function getReaplcedOrder(Request $request){
         $input=$request->all();
         	$replaced_data=DB::table('replace_order_details')->where('sub_order_id',$input['suborder_id'])->first();
         	  ...
         	  $prd_data=Products::productDetails($replaced_data->product_id);
         	$html='';
         	if($replaced_data){
         	   $html=' <table border="1">
                    <tr>
                    ...
                    if($replaced_data->w_size!=''){
                            $html.='<th>Women Size</th>
                            <td>'.$replaced_data->w_size.'</td>
                            </tr>
                            <tr>';
                    }
                   ...
                   $html.='<th>Price</th>
                    ...
                    <th>Total</th>
                    <td>'.$replaced_data->product_price*$replaced_data->product_qty	.'</td>
                    </tr>
         </table>'; 
         	}
...
if($input['owner']==0){
    ...
    $data=User::select('password','email')->where('id',$admin_id)->first();
        if($input['email']!='' && $input['email']!=null){
            if ($data && Hash::check($input['current_password'], $data->password)) {
             User::where('id',$admin_id)->update(array('password'=>Hash::make($input['new_password']),'email'=>$input['email']));
            return Redirect::route('changePassword')->withSuccess('password and email is changed');
            }else{
            return Redirect::route('changePassword')->withErrors(['Current password is not matched']);
            }
            ...
            if ($data && Hash::check($input['current_password'], $data->password)) {
               ...
               return Redirect::route('changePassword')->withSuccess('password is changed');
            } else{
            return Redirect::route('changePassword')->withErrors(['Current password is not matched']);
            }
        }    
	...
	public function customers(Request $request){
		...
		if($search!='' && $search!='all'){
				  ...
				  $customers=$customers->where(function($query) use ($search){
                    ...
                    $query->orWhere('customers.id','LIKE',$search.'%');
             });
		}
        ...
        return view('admin.customers.list',['customers'=>$customers,'page_details'=>$page_details,'isOtpVerified'=>$isOtpVerified,'status'=>$status]);   
    }
	...
	public function payment_pay_export(Request $request){

      $dt_rnge='';
      $str='';

        return Excel::download(new CustomerPaymentHistory($str,$dt_rnge), 'Payment'.date('d-m-Y H:i:s').'.csv');    
    }
    public function customers_payment(Request $request){
        
		$customers=Customer::select(
						'customers.*','orders.order_no','orders.txn_id','orders.grand_total','orders.txn_status','orders.order_date','orders.payment_mode'
						);
		
		$customers=$customers
				->join('orders','customers.id','=','orders.customer_id')
				->whereRaw('orders.txn_id <> ""')
				//->whereNotNull('orders.txn_id')
		...
		$page_details=array(
							...
							"search_route"=>'',
							"export"=>route('payment_pay_export'),
							"reset_route"=>''
					 );

        // dd($customers);
	 
        return view('admin.customers.customer_payment_list',['customers'=>$customers,'page_details'=>$page_details]);   
    }
	...
	public function customer_wallet(Request $request){
     ...
     public function customer_referals(Request $request){
        $id=base64_decode($request->id);
		...
		$page_details=array(
	 	...
	 	$customer=Customer::where('id',$id)->first();
                ...
                ->where('p_id',$id)->paginate(500);
		
		return view('admin.customers.customer_referarls',['customer'=>$customer,'page_details'=>$page_details,'referals'=>$referals]);
...
$json_data = array(
	...
	public function productEnquirry(Request $request){
	    ...
	    $dataEnquirry = DB::table('product_enquiry')->select('product_enquiry.*','product_enquiry.name as enq_name',
	    'products.vendor_id','products.name as productName','vendors.f_name','vendors.l_name')
	                    ->join('products','products.id','=','product_enquiry.product_id')
	                    ->join('vendors','vendors.id','=','products.vendor_id')
	                    ->orderBy('product_enquiry.id','DESC')
	                    ->paginate(10);
	   
        return view('admin.vendors.enquiry_list',['dataEnquirry'=>$dataEnquirry,'page_details'=>$page_details]);   
	}
