...
class AttributeController extends Controller
{
	...
	public function getCategoryAttributes(Request $request){
		...
		if($input['attributesType']==0){
		 ...
		 ->join('colors', 'colors.id', '=', 'categories_attributes.attrinbute_id')
		->where('categories_attributes.attribute_type',$input['attributesType'])
    ->where('categories_attributes.category_id',$input['cat_id'])
    ->where('colors.isdeleted',0)
    ->get();
    ...
    public function colors(Request $request)
    {
		...
		$Colors= Colors::where('isdeleted', 0)->where('id', '!=',0);
    ...
    {
			$input=$request->all();
			Colors::whereIn('id', $input['product_id'])
    ->update([
        'isdeleted' =>1
    ]);
     ...
     public function bulkActivecolor(Request $request){
         $prd_ids=explode(",",$request->product_ids);
             Colors::whereIn('id',$prd_ids)
			->update(
						[
                            'status' =>1
						  ]
...
// 		}
   ...
   public function editcolor(Request $request)
   {
	 ...
	 if ($request->hasFile('color_image')) {
			$banner_image = $request->file('color_image');
			$destinationPath2 =  Config::get('constants.uploads.color');
			$file_name2=$color_image->getClientOriginalName();
   ...
   return Redirect::back();
	...
	public function sizes(Request $request)
    {
		...
		$Sizes= Sizes::where('isdeleted', 0)->where('id', '!=',0);;
    ...
    {
			$input=$request->all();
			Sizes::whereIn('id', $input['product_id'])
    ->update([
        'isdeleted' =>1
    ]);
     ...
     public function bulkActiveSize(Request $request){
         $prd_ids=explode(",",$request->product_ids);
             Sizes::whereIn('id',$prd_ids)
			->update(
						[
                            'status' =>1
						  ]
	 ...
	 if ($request->isMethod('post')) {
       ...
       /* save the following details */
				if($error){
				MsgHelper::save_session_message('danger','Size Already exist',$request);
				} else{
				MsgHelper::save_session_message('success',Config::get('messages.common_msg.data_save_success'),$request);
				}
		 return Redirect::back();
