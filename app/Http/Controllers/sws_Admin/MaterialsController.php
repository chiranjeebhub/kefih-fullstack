...
class MaterialsController extends Controller
{
     ...
     */
    public function materials_export(Request $request)
    {
		$str=$request->str;
		$daterange=$request->daterange;
		return Excel::download(new materialsExport($str,$daterange), 'material'.date('d-m-Y H:i:s').'.csv');	
    }
    public function lists(Request $request)
    {
		$parameters=$request->str;
		$status=$request->status;
		$export=URL::to('admin/materials_export');
		$page_details=array(
       "Title"=>"Materials",
	...
	public function multi_delete_material(Request $request)
    {
			$input=$request->all();
           // dd($input);
			Materials::whereIn('id', $input['filter_id'])
    ->update([
        'isdeleted' =>1
    ]);
