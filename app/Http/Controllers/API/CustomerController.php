    ...
    public function getProfileOTP(Request $request){
		...
		CommonHelper::generate_profile_otp(array(
			...
			$res=array(
				"status"=>true,
				"statusCode"=>201,
				"message"=>"Redliips E-shop OTP sent for change profile.",
				//"isOtpVerified"=>'0',
   ...
   public function OTPlogin(Request $request)
    {
		...
		$input = json_decode(file_get_contents('php://input'), true);
		file_put_contents("ooooootp.txt",json_encode($input));
...
if ($validate_user) {
	...
	if($validate_user->status!=1){
		...
		if (filter_var($input['fld_user_phone'], FILTER_VALIDATE_EMAIL)) {
			...
			$res=array(
				"status"=>true,
				"statusCode"=>201,
				"message"=>"redlips E-shop send OTP to your register email.",
				"isOtpVerified"=>'0',
				"login_data"=>$validate_user
				);			
		echo json_encode($res);	
				...
				"status"=>true,
				"statusCode"=>201,
				"message"=>"redliips E-shop send OTP to your register mobile number continue to login.",
				"isOtpVerified"=>'0',
				"login_data"=>$validate_user
				);			
		echo json_encode($res);	
               	    ...
               	    if($validate_user->isOtpVerified==1){
                		...
                		CommonHelper::generate_user_otp($custmor_array);
                         $res=array(
                				"status"=>true,
                				"statusCode"=>201,
                				"message"=>"redlips E-shop send OTP to your register mobile number verify to continue login.",
                				"login_data"=>$validate_user
                				);			
 ...
 public function signup(Request $request)
	   ...
	   $fake_data=array(
		...
		$isemailExist= Customer::where('email',$input['fld_user_email'])->where('isdeleted',0)->first();
 	    if($isemailExist){
				$res=array(
				"status"=>true,
				"statusCode"=>404,
	...
	$isphoneExist= Customer::where('phone',$input['fld_user_phone'])->where('isdeleted',0)->first();
 	if($isphoneExist){
				$res=array(
				"status"=>true,
				"statusCode"=>404,
      ...
      if($Customer->save()){
           $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'.date('dmYHis'); 
             ...
             $email_msg='   <tr>
                         ...
                         <td style="padding:5px 10px;">
                                  <p><strong></strong>'.$Customer->name.' '.$Customer->last_name.'</p>
                         </td>
                     </tr>
                     <tr>
                         <td style="padding:5px 10px;">
      ...
      if($Customer->save()){
		  ...
		  $signup_data=array(
			...
			public function verify(Request $request) {
				    ...
				    $input = json_decode(file_get_contents('php://input'), true);
				    file_put_contents('ver.txt',json_encode( $input ));
             ...
             $record = DB::table('customer_phone_otp')->where('user_id',$input['fld_user_id'])->first();
			...
			public function resend_otp(Request $request) {
				...
				if($user){
					...
					$custmor_array=array(
					  ...
					  CommonHelper::generate_user_otpApp($custmor_array);
...
public function userProfile(){
    $input = json_decode(file_get_contents('php://input'), true);
 file_put_contents("pro.txt",json_encode($input));
    if(!$input){
                 	$res=array(
				"status"=>true,
				"statusCode"=>404,
    	 ...
    	 $validate_user= Customer::select(
					    ...
					    DB::RAW('IFNULL(dob, "") as dob'),
					     DB::RAW('IFNULL(address, "") as address'),
					       DB::RAW('IFNULL(gender, "") as gender'),
					   
					    'total_reward_points as total_reward_points',
					     DB::raw("(IF(customers.profile_pic!='', CONCAT('".$this->site_base_path."uploads/customers/profile_pic/',customers.profile_pic),'')) as fld_profile_image")
			)->where('id',$input['fld_user_id'])->first();
				if($validate_user){
