...
use Config;
class CheckOutController2 extends Controller 
{
...
//echo  '<pre>';print_r($data);die;
    ...
    public function applyCoupon(Request $request) {
		$input = json_decode(file_get_contents('php://input'), true);
		  ...
		  file_put_contents('apply'.date('Y-m-d H:i:s').'.txt',json_encode($input));
	$output=array(
            "Error"=>1,
            "Msg"=>"",
            //"cart_total"=>"",
...
if (($paymentDate >= $contractDateBegin) && ($paymentDate <= $contractDateEnd)){
  $response=array(
	...
	public function saveOrdertest2(Request $request){
...
// 				die();
     ...
     if($shipping_adddress->shipping_city!=$sitecityname){
            		...
            		"status"=>false,
            		"statusCode"=>404,
            		"message"=>"We have no coverage in your area",
            		"success_order_id"=>''
            		);
        ...
        $order_no='Redliips'.date('YmdHis');
         ...
         if($invalidProducts>0){
             	$response=array(
            		"status"=>false,
            		"statusCode"=>404,
            		"message"=>"Invalid products in your cart",
            		"success_order_id"=>''
            		);
