...
class CustomFormHelper
{
    ...
    public static function form_builder($form_array)
    {
		...
		$field_html.='<label for="exampleInputEmail1">'.$form_array['label'].'</label>';
			switch($form_array['type']){
				...
				break;
					...
					case 'text':
					$ismaxLength = (!empty($form_array['maxlength']))?'maxlength="'.@$form_array['maxlength'].'"':'';
				$field_html.=' <input type="text" name="'.$form_array['name'].'"  '.$form_array['disabled']./*'" maxlength="'.@$form_array['maxlength'].*/$ismaxLength.'" value="'.$form_array['value'].'" class="'.$form_array['classes'].'" id="'.$form_array['id'].'" placeholder="'.$form_array['placeholder'].'"   '.(array_key_exists("script",$form_array)?$form_array['script']:'').'>';
			...
			case 'text_required':
				...
				case 'select':
						$field_html.='<select class="'.$form_array['classes'].'"  name="'.$form_array['name'].'" id="'.(array_key_exists("id",$form_array)?$form_array['id']:'').'">';
				...
				case 'selectcustom':
						...
						$field_html.='<option value="">Select </option>';
								...
								foreach($form_array['value'] as $row){
									$selected_id=($row->id==$form_array['selected'])?"selected":"";
												$field_html.='<option value="'.$row->id.'" '.$selected_id.' data-id="'.$row->id.'">'.$row->name.'</option>';
								}
						$field_html.='</select>';
				...
				case 'select_with_inner_loop_for_filter':
				    	$field_html='<div class="form-group">';
						$field_html.='<select class="'.$form_array['classes'].'"  name="'.$form_array['name'].'" id="'.$form_array['id'].'" multiple>';
						$field_html.='<option value="">Select </option>';
						$field_html.=$form_array['value'];
						$field_html.='</select>';
				...
				case 'select_with_inner_loop_for_filter12':
				    	$field_html='<div class="form-group">';
						$field_html.='<select class="'.$form_array['classes'].'"  name="'.$form_array['name'].'[]" id="'.$form_array['id'].'" multiple="multiple">';
						$field_html.='<option value="">Select </option>';
						$field_html.=$form_array['value'];
						$field_html.='</select>';
		...
		$field_html.='<label for="exampleInputEmail1">'.$form_array['label'].'</label>';
				...
				case 'date':
				$field_html.=' <input type="date" name="'.$form_array['name'].'"  '.$form_array['disabled'].' value="'.$form_array['value'].'" class="'.$form_array['classes'].'" id="'.$form_array['id'].'" placeholder="'.$form_array['placeholder'].'">';
				break;
				case 'time':
					$field_html.=' <input type="time" name="'.$form_array['name'].'"  '.$form_array['disabled'].' value="'.$form_array['value'].'" class="'.$form_array['classes'].'" id="'.$form_array['id'].'" placeholder="'.$form_array['placeholder'].'">';
					break;
				
				case 'datetime':
				$field_html.='
                <div class="input-group date" id="'.$form_array['id'].'">
                    <input type="text" class="'.$form_array['classes'].'" name="'.$form_array['name'].'" value="'.$form_array['value'].'"  placeholder="'.$form_array['placeholder'].'">
		...
		$field_html.='<input type="file" name="'.$form_array['name'].'" id="'.$form_array['id'].'" class="'.$form_array['classes'].'" data-multiple-caption="{count} files selected" />';
		   ...
		   break;
				...
				case 'number':
				$field_html.=' <input type="number" name="'.$form_array['name'].'" '.$form_array['disabled'].'  value="'.$form_array['value'].'" class="'.$form_array['classes'].'" id="'.$form_array['id'].'" maxlength="'.@$form_array['maxlength'].'" min="'.@$form_array['min'].'" placeholder="'.$form_array['placeholder'].'">';
				break;
				
				
				case 'password':
				$field_html.=' <input type="password" name="'.$form_array['name'].'"  value="'.$form_array['value'].'" class="'.$form_array['classes'].'" id="'.$form_array['id'].'" placeholder="'.$form_array['placeholder'].'">';
				break;
	 ...
	 public static function support_image($path,$name)
    {
		$path2 = '';
		if($name!=''){
			  $path2=Config::get('constants.Url.public_url').$path.'/'.$name;
		} else{
			 $path2=Config::get('constants.Url.public_url').Config::get('constants.no_images');
		}
      
		
        return  $image='<img src="'.$path2.'" class="img-thumbnail" alt="'.$name.'" width="70" height="70">';
    }
	public static function signature_pic($path,$name)
    {
        ...
        return  $image='<img src="'.$path2.'" class="img-thumbnail" alt="'.$name.'" width="50" height="50">';
    }
