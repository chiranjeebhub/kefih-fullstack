...
$usid = Request::segment(3);
 ?>
	<div class="allbutntbl">
		<a href="{{route('customers') }}" class="btn btn-default"><i class="fa fa-undo"></i> Back</a>
	</div>
	<div class="mailbox-read-info">
	<h3>@if(base64_decode($usid)==0) Highest selling product @endif
						...
						<tr>
								...
								<td>
								    @if($row['order_shipping_zip'])
								     <span class="badge bg-danger">Location: {{$row['order_shipping_zip']}},{{$row['order_shipping_city']}}</span>
								     <br>	
								    @endif
							
							 
								<img src="{{url('/')."/uploads/products/".$row['default_image']}}" style="height:50px; width:50px;">
