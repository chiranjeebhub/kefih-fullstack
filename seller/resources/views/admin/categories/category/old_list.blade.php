...
<form role="form" class="form-element multi_delete_form mt15" action="{{route('multi_vendor_delete_cat')}}" method="post">
  @csrf
					  ...
					  @foreach($Categories as $Category)
						...
						<tr>
						 <td><input type="checkbox" class="checkbox multiple_select_checkBox" name="cat_id[]" value="{{$Category->id}}"></td>
							...
							{
							?>
							<a href="{{ route('add_sizechart',base64_encode($Category->id)) }}" class="btn btn-success">Update size chart</a>
							<?php }else{
								?>
								<a href="{{ route('add_sizechart',base64_encode($Category->id)) }}" class="btn btn-success">Add size chart</a>
								<?php } ?>
