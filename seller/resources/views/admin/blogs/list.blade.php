...
<div class="">
	<div class="allbutntbl">
		<a href="{{ route('addblog') }}" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i> Add Blog</a>
	</div>
	<div class="col-sm-12">
		<div class="row">
			<div class="col-sm-2">
...
<form role="form" class="form-element multi_delete_form mt15" action="{{route('multi_delete_blog')}}" method="post">
  @csrf
				  ...
				  <table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
					  ...
					  @foreach($Blogs as $Blog)
						...
						<tr>
						 <td><input type="checkbox" class="checkbox multiple_select_checkBox" name="blog_id[]" value="{{$Blog->id}}"></td>
								<td>{{$Blog->name}}</td>
								<td>{!! App\Helpers\CustomFormHelper::support_image('uploads/blog/banner',$Blog->banner_image); !!}</td>
								
							<td>
							<a href="{{route('blog_sts',[base64_encode($Blog->id),base64_encode($Blog->status)] )}}"
									...
									&nbsp;|&nbsp;
							<a href="{{route('editblog', base64_encode($Blog->id))}}">
								<i class="fa fa-pencil text-blue" aria-hidden="true"></i>
								</a>&nbsp; | &nbsp;
							<a href="{{route('deleteblog', base64_encode($Blog->id))}}"
