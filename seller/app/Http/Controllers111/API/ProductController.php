...
->join('categories','product_categories.product_id','=','products.id')
    ...
    /* public function product_filter(Request $request){
	 ...
	 file_put_contents('product.txt',json_encode($input));
		...
		foreach($record as $row){
		    $color_image=ProductImages::getConfiguredImagesAPI($row['id'],$row['color_id']);
                ...
                // $row['default_image']="https://www.phaukat.com/uploads/products/".$row['gif_image'];
                 /////$row['default_image']=$prd_img;
                $row['gif_image']="https://www.phaukat.com/uploads/products/".$prd_img;
				$row['price']=$row['price'];
				$row['spcl_price']=$spcl;
    ...
    public function product_detail(Request $request) {
		...
		$attributes=array('sizes'=>$size_record,'women_sizes'=>$women_size_record,'colors'=>array_reverse($color_record));
		$more_sellers=DB::table('vendor_existing_product')
						->join('vendors','vendor_existing_product.vendor_id','vendors.id')
						->where('vendor_existing_product.master_product_id','=',$product_id)
						->get();
		...
		$spcl=0;
		 $spcl=$record->price+$extrs_price;
            if($record->spcl_price!=''){
...
// 		$prd_img='';
    ...
    public function home_product_listing(Request $request) {
		...
		$input = json_decode(file_get_contents('php://input'), true);
		file_put_contents("home.txt",json_encode($input));
		...
		if($page!=0){
			...
			$record =Products::select(
						...
						->join('product_attributes', 'products.id', '=', 'product_attributes.product_id')
						->join('product_categories','products.id','=','product_categories.product_id')
						->where('product_categories.cat_id','=',$rootcat_id)
				// 		->where('product_attributes.qty','>',0)
						->where('products.status',1)
						->where('products.isdeleted',0)
	 ...
	 public function recentlyViewd(Request $request) {
		...
		$input = json_decode(file_get_contents('php://input'), true);
		file_put_contents("rece111.txt",json_encode($input));
