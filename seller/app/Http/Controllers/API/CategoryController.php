...
class CategoryController extends Controller 
{
    ...
    public function category_listing(Request $request) {
		...
		$res=array(
                    ...
                    "fld_total_page"=>ceil(sizeof($Allrecord)/6),
					"message"=>$message,
	...
	public function category_offer_listing(Request $request) {
		...
		$record =DB::table('offer_categories')
						->select(
									...
									'offer_categories.categories_id',
		...
		if($record){
			$statusCode=201;
			$message="Offer Category Listing";
			$category_offer_data=$record;
			$return_page=($page+1);
		}else{
			$statusCode=404;
			$message="No Offer Category Found";
