...
class OrderController extends Controller 
{
	   ...
	   if($page!=0){
		 ...
		 $sub_orders=DB::table('order_details')
						  ->select(
								...
								/*'orders.deduct_reward_points as fld_order_deduct_reward_points',
								'orders.coupon_code as coupon_code',
								'orders.coupon_percent as coupon_percent',
	   ...
	   $prd_img=$this->site_base_path.'uploads/products/';
        ...
        $sub_orders=DB::table('order_details')
              ->select(
						...
						/*'orders.order_deduct_reward_points as fld_order_deduct_reward_points',
						'orders.coupon_code as coupon_code',
						'orders.coupon_percent as coupon_percent',
						'order_details.order_coupon_amount as coupon_amount',*/
						'order_details.suborder_no as fld_suborder_id',
    ...
    public function order_track(Request $request){
		...
		/*$inputs['docket_no']='02249TEST7965';
		
		$ss=XpressbeesHelper::OrderTrack($inputs);
		$track_data=$ss->GetBulkShipmentStatus[0];*/
		
		$track_data=DB::table('order_track')->where('fld_order_detail_id','=',$id)->get()->toArray();
		
		$page=0;
		$allrecord=1;
		$message="Order Track ";
		$api_key='order_track_data';
		
		echo $this->msg_info($message,$track_data,$page,$api_key,$allrecord);
    }
