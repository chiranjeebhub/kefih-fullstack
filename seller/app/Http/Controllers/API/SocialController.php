...
class SocialController extends Controller {
	...
	public function savedata(Request $request){
		//echo "jyoti";die;$destinationPath = public_path(sprintf("\\uploads\\%s\\", str_random(8)));
//$uploaded = Storage::put($destinationPath. $fileName, file_get_contents($file->getRealPath()));
		...
		if($validate_email){
            ...
            if($Customer->save()){
            	...
            	$url=$this->site_base_path.Config::get('constants.uploads.customer_profile_pic').'/'.$Customer->profile_pic;
