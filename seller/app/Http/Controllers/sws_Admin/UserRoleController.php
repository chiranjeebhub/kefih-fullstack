...
class UserRoleController extends Controller
{
     ...
     public function delete_users(Request $request)
    {
        ...
        $input['admin_id']=array($id);
          $admins_data=DB::table('users')->select('phone')->whereIn('id',$input['admin_id'])->get();
            $admin_phone=array();
              foreach($admins_data as $admin){
            array_push($admin_phone,$admin->phone);
        }
             if(sizeof($admin_phone)>0){
                 DB::table('vendor_admin_forgot_password')
                ->whereIn('phone',$admin_phone)
                ->delete();
        }
