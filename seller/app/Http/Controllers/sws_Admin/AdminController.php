...
class AdminController extends Controller
{
    ...
    public function vendor_rating(Request $request){
        	...
        	$page_details=array(
				"Title"=>"Vendor Rating",
				"Box_Title"=>"Vendor Rating",
				"search_route"=>'',
				 "export"=>'',
				"reset_route"=>''
     );
        	$parameters=base64_decode($request->v_id);	
        	$ratings=DB::table('vendor_rating')->select('vendor_rating.*','vendors.public_name','customers.name','customers.last_name')->join('vendors','vendor_rating.vendor_id','vendors.id')
        	->join('customers','vendor_rating.user_id','customers.id')
        	->where('vendor_rating.vendor_id',$parameters)->paginate(9);
    return view('admin.vendors.vendor_rating',['page_details'=>$page_details,'ratings'=>$ratings]);
        
    }
    
    public function vdr_rt__sts(Request $request)
    {
            $id=base64_decode($request->id);
            $sts=base64_decode($request->sts);

            $res=DB::table('vendor_rating')->where('id',$id)
                    ->update(['isActive' => ($sts==0) ? 1 : 0]);
    ...
    public function index(Request $request)
    {
        ...
        $vendors= Vendor::where('isdeleted',0);
		...
		$vendors=$vendors->where('isdeleted',0)->orderBy('id', 'DESC')->paginate(100);
    ...
    {
			$input=$request->all();
				Vendor::onChangeStsDelete($input['vendor_id'],1);
    ...
    ->update([
     ...
     $vendors_data=DB::table('vendors')->select('phone')->whereIn('id',$input['vendor_id'])->get();
            $vendor_phone=array();
            foreach($vendors_data as $vendor){
            array_push($vendor_phone,$vendor->phone);
        }
        
         if(sizeof($vendor_phone)>0){
            
            DB::table('phone_otp')
            ->whereIn('phone',$vendor_phone)
            ->delete();
 ...
 $page_details=array(
	  ...
	  if ($request->isMethod('post')) {
			 ...
			 $request->validate([
				...
				'profile_pic' => 'mimes:jpeg,png|min:'.Config::get('constants.size.vendor_profile_image_min').'|max:'.Config::get('constants.size.vendor_profile_image_max').''    
 ...
 DB::table('vendor_categories')->insert(
      ...
      if($Vendor->updateCategories($input,$request->session()->get('vendor_id'))){
		  ...
		  return redirect()->route('add_vendor', ['level' => base64_encode(2)]);
			...
			'company_state' => 'max:100|regex:/^[a-zA-Z\s]*$/',
			'company_city' => 'max:100|regex:/^[a-zA-Z\s]*$/',
			'company_pincode' => 'string|min:6|max:6',
			'company_about' => 'string|max:360',
			'about_us' => 'max:200',
	 ...
	 if ($request->hasFile('company_logo')) {
      ...
      if($Vendor->updateMetaInfo($input,$request->session()->get('vendor_id'))){
		  MsgHelper::save_session_message('success',Config::get('messages.common_msg.data_save_success'),$request);
		   return redirect()->route('add_vendor', ['level' => base64_encode(5)]);
			...
			$request->validate([
				'ac_no' => 'string|max:25',
				'bank_name' => 'max:50|regex:/^[a-zA-Z\s]*$/',
				'branch_name' => 'max:25|regex:/^[a-zA-Z0-9\s]+$/',
				'bank_city' => 'max:25|regex:/^[a-zA-Z\s]*$/',
				'ifsc_code' => 'max:25|regex:/^[a-zA-Z0-9\s]+$/',
      ...
      if($Vendor->updateBankInfo($input,$request->session()->get('vendor_id'))){
		...
		$request->validate([
				'gst_no' => 'min:15|max:15|regex:/^[a-zA-Z0-9\s]+$/',
				'pan_no' => 'min:10|max:10|regex:/^[a-zA-Z0-9\s]+$/',
				 
                //'gst_no' => 'min:15|max:15',
                //'pan_no' => 'min:10|max:10|alpha_num',
                
				'gst_file' => 'mimes:jpeg,png|min:'.Config::get('constants.size.vendor_gst_image_min').'|max:'.Config::get('constants.size.vendor_gst_image_max').'',
			 ...
			 break;
			  case 1;  // categories tab
				$Vendor = new Vendor;
						...
						{   
								MsgHelper::save_session_message('danger','Please select category',$request);
      ...
      if($Vendor->updateCategories($input,$request->session()->get('vendor_id'))){
		  ...
		  return Redirect::route('edit_vendor', [base64_encode(1),base64_encode($id)]);
			...
			'name' => 'max:25',
                ...
                'company_state' => 'max:100|regex:/^[a-zA-Z\s]*$/',
                'company_city' => 'max:100|regex:/^[a-zA-Z\s]*$/',
                'company_pincode' => 'string|min:6|max:6|regex:/[1-9][0-9]{5}/',
				'company_about' => 'string|max:360',
			'about_us' => 'max:200',
...
$failed=$failed->where('order_details.order_status',6)->count();
  ...
  $st=($sts==0) ? 1 : 0;
            ...
            if ($res) {
                if($st==0){
                $input['vendor_id']=array($id);
                	Vendor::onChangeStsDelete($input['vendor_id'],0);
                }
                MsgHelper::save_session_message('success',Config::get('messages.common_msg.data_save_success'),$request);
            } else {
                MsgHelper::save_session_message('danger',Config::get('messages.common_msg.data_save_error'),$request);
    	...
    	public function vdr_term_sts(Request $request)
    {
            $id=base64_decode($request->id);
            $sts=base64_decode($request->sts);

            $res=Vendor::where('id',$id)
                    ->update(['term_accepted' => ($sts==0) ? 1 : 0]);
