...
class ProductController extends Controller 
{
 ...
 $page_details=array(
	  ...
	  if ($request->isMethod('post')) {
			...
			$input=$request->all();
			file_put_contents("images.txt",json_encode($input));
			$request->validate([
			...
			if($request->hasfile('images'))
				 {
					...
					{
						$destinationPath =Config::get('constants.uploads.product_images');
						$file_name=auth()->guard('vendor')->user()->id.$image->getClientOriginalName();
							$fn= FIleUploadingHelper::BulkUploadIMages($destinationPath,$image,$file_name,0,0,true);
							if($fn!=''){
							    $image_cc++;
							}
               ...
               // return view('admin.product.image_uploads',['page_details'=>$page_details]);
 ...
 $page_details=array(
		...
		"Form_data"=>array(
         ...
         "product_qty_for_out_stock_field"=>array(
							'label'=>"Qty for item to be notified",
							'type'=>'number',
							'name'=>'qty_out',
							'id'=>'qty_out',
							'classes'=>'form-control',
		 ...
		 MsgHelper::save_session_message('success',Config::get('messages.common_msg.data_save_success'),$request);
			  ...
			  $request->validate([
                ...
                'stock_availability' => ''
                        ...
                        'qty.lt'=>'Quantity for notify should  be less than Quantity '
                    ]
			);
...
// 			'product_brand' => 'required',
	...
	public function edit_product(Request $request)
   {
        ...
        $id=base64_decode($request->id);
       $colors= CommonHelper::getColors($id);
 ...
 $page_details=array(
	  ...
	  if ($request->isMethod('post')) {
			 ...
			 if($Products->isexisting==1){
				...
				// 'sku' => 'required|max:25',
				// 'product_code' => 'required|max:25',
				//  'gtin' => 'max:500|unique:products,gtin,'.$id.',id,isdeleted,0',
				'weight' => 'max:25',
				'height' => 'max:25',
				'length' => 'max:25',
				'width' => 'max:25',
...
// 			'product_brand' => 'required',
			...
			$extras=array(
                            ...
                            'product_brand' => !empty($input['product_brand']) ? $input['product_brand'] : '',
...
//                     Excel::load($request->file('csv')->getRealPath(), function ($reader) {
...
// 			die();
			...
			$filepath = Config::get('constants.Url.public_url2').Config::get('constants.uploads.csv').'/'.$res;
		  ...
		  $j=0;
		   $vdr_id=auth()->guard('vendor')->user()->id;
                 ...
                 $upload_error=array();
                // file_put_contents("sheet_data.txt",json_encode($importData_arr));
					...
					if($j>0){
					  ...
					  // {
								...
								// 			21 pr BTA point
									...
									$lastId = $Product->id;
									 if($importData[19]!=''){
                                        ...
                                        $ProductImages = new ProductImages;
                                        $ProductImages->updateVDRProductImages($rel_data,$lastId,auth()->guard('vendor')->user()->id);
                                        }
							...
							/*} else{
								...
								// 17/05/23
								$Product = Products::where(['vendor_id'=>auth()->guard('vendor')->user()->id,'id'=>$res->id])->first();
								$Product->vendor_id = auth()->guard('vendor')->user()->id;
								...
								// 17/05/23
							     // array_push($upload_error,array('row'=>$j,'error_message'=>$importData[0].' Product  exists')); 
									...
									/*Simple Product*/
								  	...
								  	$res=Products::select('id','vendor_id')->where('isdeleted',0)
                                                            ->where(function($query) use($importData){
                                                                $query->where('name', $importData[0]);
                                                                // $query->orwhere('gtin', $importData[3]);
                                                            })
							...
							if($res){
							   ...
							   if($res->vendor_id==auth()->guard('vendor')->user()->id){
	...
	public function enquirry_vendor(Request $request){
	 
	   $id= auth()->guard('vendor')->user()->id;
	    $page_details=array(
							"Title"=>"Product Enquiry",
							"Box_Title"=>"Product Enquiry(s)",
					 );
	    
	    $dataEnquirry = DB::table('product_enquiry')->select('product_enquiry.*','products.vendor_id','products.name as productName','vendors.f_name','vendors.l_name')
	                    ->join('products','products.id','=','product_enquiry.product_id')
	                    ->join('vendors','vendors.id','=','products.vendor_id')
	                    ->where('vendors.id',$id)
	                    ->orderBy('product_enquiry.id','DESC')
	                    ->paginate(10);
	   
        return view('admin.enquiry_list',['dataEnquirry'=>$dataEnquirry,'page_details'=>$page_details]);   
	}
	...
	public function delete_enquiry(Request $request)
    {
		...
		$res=DB::table('product_enquiry')->where('id',$id)->delete();
	...
	public function uploadzipfile(Request $request)
	{
		...
		$storepath=base_path("/uploads/products/".$vendorid);
		$storepathold = explode('/', $storepath);
		unset($storepathold[5]);
		$storepatholdnew=implode("/",$storepathold);

		$foldername=base_path("/uploads/products/");
		$dir_name =$foldername.''.$vendorid;
		
		$items = explode('/', $dir_name);
		unset($items[5]);
		$newurl=implode("/",$items);

		if (!is_dir($storepatholdnew)) {
		mkdir($newurl);
		}
		$zip = new ZipArchive();
		...
		if ($status === true) {
			...
			$storageDestinationPath= $storepatholdnew;
			$zip->extractTo($storageDestinationPath);
			$zip->close();
			MsgHelper::save_session_message('success','You have successfully extracted zip',$request);
			return Redirect::back();
	...
	public function  getfoldername(Request $request)
	{
		...
		$data=[];
		$i = 0;
		$dir = base_path().'/uploads/products/'.$vendorid;
		$storepathold = explode('/', $dir);
		unset($storepathold[5]);
		$storepatholdnew=implode("/",$storepathold);

		$publicpath=public_path();
		$dirList = scandir($storepatholdnew);
		foreach ($dirList as $key => $value) {
			$rountname=route('viewspageimages',[$vendorid,$value]);
	...
	public function  serachfoldername(Request $request)
	{
		...
		$data=[];
		$i = 0;
		$dirnew = base_path().'/uploads/products/'.$vendorid;

		$dirnewold = explode('/', $dirnew);
		unset($dirnewold[5]);
		$dir=implode("/",$dirnewold);
	...
	public function deletefolder(Request $request)
	{
		$data=[];
		$vendorid=auth()->guard('vendor')->user()->id;
		$folderPathnew=base_path().'/uploads/products/'.$vendorid.'/'.$request->foldername;

		$dirnewoldold = explode('/', $folderPathnew);
		unset($dirnewoldold[5]);
		$folderPath=implode("/",$dirnewoldold);
	...
	{
		$vendorid=auth()->guard('vendor')->user()->id;
		$dirnew = base_path().'/uploads/products/'.$vendorid.'/'.$folderid;
	...
	{
		$vendorid=auth()->guard('vendor')->user()->id;
		$folderPathnew=base_path().'/uploads/products/'.$vendorid.'/'.$request->folderid;
		$dirnewoldold = explode('/', $folderPathnew);
		unset($dirnewoldold[5]);
		$folderPath=implode("/",$dirnewoldold);
		...
		return redirect()->route('viewspageimages',[$vendorid,$request->folderid])->with('success','Selected iamges delete successfully.');;
		exit();
	...
	public function viewProductImages(Request $request){
		...
		return view('admin.product.image_view',[
			'page_details'=>$page_details,'pagefolder'=>$dirList,'vendorid'=>$vendorid
			]);
	 }
