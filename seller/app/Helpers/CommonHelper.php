...
public static function validVendorCount(){
    $total=Vendor::select('id')->where(['isdeleted'=>0, 'status' => 1])->get();
    return sizeof($total);
}

public static function validCustomerCount(){
    $total=Customer::select('id')->where(['isdeleted'=>0, 'status' => 1])->get();
return sizeof($total);
} 
...
// 		$data['message']=$msg;
	...
	public static function sendEmail($data)
    {
                     try {
                           ...
                           $mail = new PHPMailer();
                            $mail->IsSMTP();
                            $mail->Host = 'mail.redliips.com';
                            $mail->SMTPAuth = true;
                            $mail->SMTPSecure = "ssl";
                            $mail->Port ='465';
                            $mail->Username = "mailto@redliips.com";
                            $mail->Password = "tomail$#@!";
                            
                            $mail->From = 'mailto@redliips.com';
...
return $string;
	...
	public static function vendor_forgot_password($data)
    {
            ...
            'email_otp'=>$otp
                ...
                $data['message']='Use OTP:'.$otp.' to verify your email';
                
                $data['body']='Use OTP:'.$otp.' to verify your email';
                $data['subject']='OTP';
                $data['to']=$data['Email'];
                self::SendmailCustom($data);
            // self::sendEmail($data);
