...
<?php 
    ...
    $vendor_categories = DB::table('vendor_categories')->where('vendor_id',$vendorID)->first(); 
     if(!empty($vendor_categories)){
       $vendorCats = explode(",",$vendor_categories->selected_cats);
     }
    ...
    if(!empty($vendorCats)){
        ...
        ->join('product_categories', 'categories.id', '=', 'product_categories.cat_id')
        ->join('products', 'products.id', '=', 'product_categories.product_id')
        ->whereIn('categories.id',$vendorCats)
	...
	$color_data = App\Products::select('colors.*')
								...
								->whereIn('product_categories.cat_id',$vendorCats)
								->where('products.isdeleted','=',0)
                                 ->where('products.status','=',1)
								->groupBy('colors.id')
								->orderBy('colors.name')
...
<div class="col-xs-12 col-sm-3 col-md-3 col-lg-3">
  ...
  */ ?>
                    ...
                    <div class="accordion-item">
                        ...
                        <ul class="category-list list-unstyled">
                            <li>
                                <a href="{{route('brandproducts', base64_encode($vendorID))}}" class="{{empty($cat_id)?'active':''}}">All</a>
                            </li>	
                            <?php for($i=0;$i<count($cat_data); $i++){
                            $cat_name = preg_replace('/\s+/', '-', strtolower($cat_data[$i]['name']));
                            $url=route('rbrandproducts', [base64_encode($vendorID),$cat_name,base64_encode($cat_data[$i]['id'])]);?>
                            <li>
                                <a href="{{$url}}" class="{{($cat_id == $cat_data[$i]['id'])?'active':''}}"><?php echo ucwords($cat_data[$i]['name']);?></a>
                            </li>			
                            <?php } ?> 
                        </ul>
...
{   	
	...
	$.ajax({
		type:'POST',
		data:dataString,
		url:its_url+'vendorprdlistingfilter',
		success:function(data) {
...
var htp=$(location).attr('protocol');
var urlname=$(location).attr('host');
var its_url='http://b2cdomain.in/kefih/';
var token="{{ csrf_token() }}";
$( document ).ready(function() {
	var mcat="{{($cat_id)?$cat_id:'0'}}";
...
$('.slider-time2').html(maxPrice);
   ...
   const queryString1 = window.location.search;
    ...
    var finalUrl='vendorprdlistingfilter';
...
{ 
	...
	var facilitiesid = ['brand'+'{{$vendorID}}'];
	var mcat="{{($cat_id)?$cat_id:'0'}}";
    var srt=4;
                if ($("input[name='sotyBY']:checked").val()) {
                    showClearButton();
...
$.ajax({
		type:'POST',
		data:dataString,
        url:'{{URL::to("vendorprdlistingfilter")}}',
		success:function(data) {
		    req=0;
		                $('#loader').css("display","none");
