		...
		<div style="width:100%; margin-left:auto; margin-right:auto; margin-top: 10px; margin-bottom: 10px;">
			...
			<table cellpadding="4px" cellspacing="0" style=" border: solid 1px #000; width:100%; font-family: 'Be Vietnam', sans-serif; padding:10px; background: #fff; font-size: 13px; line-height:18px;">
				...
				<tr>				     
					...
					<td>{{number_format($tax_amount+$serviceCharge,2)}}</td>
				</tr>
				<tr>
					<td colspan="6">&nbsp;</td>
				</tr>
				<tr  bgcolor="#ebecec">				     
					...
					<td><strong>{{number_format($serviceCharge,2)}}</strong></td>
					<td><strong>{{number_format($tax_amount,2)}}</strong></td>
					<td><strong>{{number_format($tax_amount+$serviceCharge,2)}}</strong></td>
				</tr>
                <tr>
				...
				<table cellpadding="2" cellspacing="0"  style="width:100%; text-align:center; border-bottom: solid 1px #000;" border="1">
                    <thead>
                        <tr>       
                          ...
                          @endif 
                          <th style="text-align:center;">Total Tax Value</th>
                    ...
                    <tbody>
                        ...
                        @if($kefih_state == $Order->order_shipping_state)  
                           @php 
                            $taxpercent = $tax/2;
                            $taxAmount = ($serviceCharge * $taxpercent)/100;
                           @endphp 

                            <td style="border-bottom: solid 1px #000;">{{$taxpercent}}</td>
                            <td style="border-bottom: solid 1px #000;">{{$taxAmount}}</td>

                           <td style="border-bottom: solid 1px #000;">{{$taxpercent}}</td>
                            <td style="border-bottom: solid 1px #000;">{{$taxAmount}}</td>
                        @else       
                           <td style="border-bottom: solid 1px #000;">{{$tax}}%</td>
                           <td style="border-bottom: solid 1px #000;">{{($serviceCharge * $tax)/100}}</td>
                        @endif 
                        <td style="border-bottom: solid 1px #000;">{{($serviceCharge * $tax)/100}}</td>
                        </tr>
                    </tbody>
				...
				<tr>
					...
					<td colspan="2">
					Total invoice Value:<strong>{{number_format($tax_amount+$serviceCharge,2)}} </strong>
					</td>
				...
				<tr>					
					...
					<p style="margin: 0;">Total in words: 
						<strong>{!! App\Helpers\CommonHelper::convert_number_to_words(round($tax_amount+$serviceCharge)); !!} 
                            Rupee Only</strong></p>
					</td>
				</tr>		
				<tr>
					<td colspan="2" style="text-align:right;">Kefih Signature</td>
				</tr>
				<tr>
