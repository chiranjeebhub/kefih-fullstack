		...
		<div class="row searchmain-row">
			...
			<ul class="list-inline disable-btn-ul">
                        ...
                        <button type="button" class="btn btn-danger btnSubmitTrigger commonClassDisableButton" disabled>Bulk Delete</button>
                        					...
                        					<li>
                        <form role="form" action="{{route('bulkActivecolor')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
				  ...
				  <table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
					  ...
					  @foreach($Colors as $Color)
						...
						<tr>
						 <td><input type="checkbox" class="checkbox multiple_select_checkBox checkedProduct" name="product_id[]" value="{{$Color->id}}"></td>
								<td>{{$Color->id}} </td>
								<td>{{$Color->name}} </td>
