                ...
                <table cellpadding="0" cellspacing="0" style="padding: 15px; background: #fff; width:100%; margin: -5px auto;">
                    <tr bgcolor="#fffff">
						<td style="text-align: center; padding: 5px 0;">
							...
							<a href="{{url('/')}}"><img width="200px;" alt="Keifh" src="{{ asset('public/fronted/images/logo.png') }}"></a>
						</td>
					</tr>
                    ...
                    <tr>
                        <td>
                            <table cellpadding="0" cellspacing="0" style="width: 100%;">
								...
								<div><p style="text-align:center; "><img width="120px" src="{{ asset('public/fronted/images/confirmed-icon.png') }}"></p>
                                            ...
                                            <tr>
												<td>
													<p style="margin: 10px 0;">Hi {{ucfirst($customer_info->name)}},</p>
													<p style="margin: 10px 0;">We're preparing your bag of joy, Containing the products you picked. Once we've shipped your order, you'll get an email with your shipping and tracking information.</p>
													<p style="margin: 10px 0;">We know you can't wait to get your hands on it, So we've begun prepping for it right away.</p>
													<p style="margin: 10px 0;">In the meantime, you can track your order.</p>
													<p style="margin: 15px 0 0;">Thank you,</p>
													<p style="margin: 0;"><strong><a href="www.kefih.com">www.kefih.com </a></strong></p>
												</td>
											</tr>
                                            <tr>
                                                <td colspan="2" style="font-size: 16px; text-align:left; border-top: solid 2px #ff000085;">
												<div style="display: inline-block; width: 100%; text-align: center; margin: 20px 0;"><a href="{{route('myorder',base64_encode(0)) }}" style="background:#e80000; color:#fff; text-decoration: none; border-radius:3px; padding:7px 20px;">Track Order</a></div>	
                                            ...
                                            <?php $subtotal=0; foreach($product_info as $products){ ?>
                                             <?php 
                               $prd_info=DB::table('products')->where('id',$products->product_id)->first();              
                            
								//Product Image setup				
							   if($prd_info->product_type == '1' )
								{
									$imagespathfolder='uploads/products/'.$prd_info->vendor_id.'/'.$prd_info->sku;
								}else{
									$productskuid=DB::table('product_attributes')->where('product_id',$prd_info->id)->first();
									$imagespathfolder='uploads/products/'.$prd_info->vendor_id.'/'.$productskuid->sku;
								}

								if($products->color_id != 0 && $prd_info->product_type==3 )
								{
										$color_price=DB::table('product_attributes')
										->where('product_id',$prd_info->id)
										->where('color_id',$products->color_id)->first();									
										$color_image=App\ProductImages::getConfiguredImages($prd_info->id,$products->color_id,0);										
										$url=URL::to($imagespathfolder).'/'.$color_image[0]['image'];								
										
								}else{								
										$prd_slider=App\Products::prdImages($product_id);
										$url=URL::to($imagespathfolder).'/'.$prd_slider[0]['image'];										
								}
...
$url=URL::to('/uploads/products/240-180').'/'.$colorwise_images->product_config_image;
        ...
        if(@$colorwise_images->product_config_image){
			$url=Config::get('constants.uploads.product_images').$colorwise_images->product_config_image; 
		} else{
			$url=URL::to('/uploads/products').'/'.$products->default_image; 
														...
														<tr>
															...
															<td style="font-size: 16px; padding: 7px 10px; vertical-align: top;">
																...
																<!--<p style="margin:0 0 10px;"><i class="fa fa-inr"></i> 50.00</p>-->
																	...
																	<p style="margin:0 0 10px;"><strong>Rs. {{@$p_price}}</strong></p>
				...
				<span style="color: #1aa4bb;">Rs. {{@$extra_info->total_shipping_charges}}</span>
						...
						<?php } ?>
						 ...
						 <?php  if(@$extra_info->service_charge!=''){ ?>	
							<p style="margin: 0 0 10px; font-size:16px; color:#6a6969;">Service Charge<span style="text-align: right; float: right;"> 
						<span style="text-align: right; float: right;" >Rs. {{@$extra_info->service_charge}}</span>
								</span>
								</p>
						<?php } ?>
			 ...
			 <?php if(@$extra_info->deduct_reward_points!=''){ ?>
					<p style="margin: 0 0 10px; font-size:16px; color:#6a6969;">Wallet Amount Used<span style="text-align: right; float: right;"> 
				...
				<span style="color: #1aa4bb;">Rs. {{@$extra_info->deduct_reward_points}}</span>
							...
							<?php } ?>				
															...
															<p style="margin: 15px 0 10px; font-size:16px; color:#000; font-weight:bold; border-top: solid 1px #ccc; padding: 10px 0 0px;">Total<span style="text-align: right; float: right;">Rs. {{$extra_info->grand_total}} </span></p>
											...
											<tr>
												<td>
													...
													<p style="margin:10 0px 15px; font-size:16px; color: #6a6969;">We wills send you a confirmation once your bag of joy is out for delivery.<br/> If you want to reach us, please <a href="{{route('contact')}}" target="_blank" style="color: #000;">Contact Us</a> here.</p>
					...
					<tr>
						<td>
						    <p style="font-size:18px; line-height:24px;">E-mail us: <a style="color:#000;" href="mailto:contact@kefih.com">contact@kefih.com</a></p>
						    <p style="font-size:18px; line-height:24px;">Customer Care: <a style="color:#000;" href="#">+91-9037538402</a></p>
						{{--
...
margin-left: 10px;">
							<a href="https://twitter.com/" target="_blank"><img title="Twitter" width="30px;" src="{{ asset('public/fronted/images/twitter-icon.png') }}"></a> 
							<a href="https://www.snapchat.com/" target="_blank"><img title="Snapchat" width="30px;" src="{{ asset('public/fronted/images/snapchat-icon.png') }}"></a> 
							<a href="https://www.facebook.com/kefihlife" target="_blank"><img title="Facebook" width="30px;" src="{{ asset('public/fronted/images/facebook-icon.png') }}"></a> 
                            <a href="https://www.instagram.com/?hl=en" target="_blank"><img title="Instagram" width="30px;" src="{{ asset('public/fronted/images/instagram-icon.png') }}"></a>
                            </span></p>
							--}}
						</td>
					</tr>
