@if (Auth::guard('customer')->check())
	...
	$(document).ready(function() { 
	    ...
	    $('.detail-gallery').each(function(){ 
	        ...
	        $(this).find(".carousel").jCarouselLite({ btnNext: ".nextMe", btnPrev: ".prevMe", speed: 400, visible:4, vertical:true, circular: true, start: 0, scroll: 1 });
	        }); 
	    
	}
	    
	    
	    var pincode = $.cookie("pincode"); var shipping_address_id = $.cookie("shipping_address_id"); var pincode_error = $.cookie("pincode_error"); if(pincode){ $('#pincode').val(pincode); if(pincode_error==1){ $('#pincode_msg').html('No delivery available in your port code'); $('.noProblem').hide(); $('.outOFdelivery').show(); } else{
    ...
    function shippinginfo(){ $('#phuakatCoinsModal').modal('show'); var html="<p>Free Shipping Available In Order Total Above <i class='fa fa-rupee'></i> "; $(".phuakatCoinsResponse").html(html); } 
    ...
    $(".phaukatCoinsInfo").click(function(){ 

        $('#phuakatCoinsModal').modal('show');
    ...
    function update_cart(wallet=1){
        ...
        else{ if(Walletkey==1){ $("#checkboxwallet1").attr('checked', true); } 
        ...
        data:{ "wallet":useWallet, "paymentMethod":paymentMethod,"slotprice":slotprice1}, 
        ...
        $('.productIncart').show(); $('.noProductIncart').hide(); } else{ $('.noProductIncart').show(); 
        $('.productIncart').hide(); } if(response.out_of_stock>0 && response.pincode_error==0){ $('.outStock').show(); $('.inStock').hide(); 
        $('.outofdelivery').hide(); } else if(response.out_of_stock==0 && response.pincode_error==1){ $('.outStock').hide(); 
        $('.outofdelivery').show(); $('.inStock').hide(); } 
           ...
           $('.discount_reponse').html(response.discount);
            ...
            $('#tax').val(response.tax);
             $('#discount_amount').val(response.discount);
              ...
              $('#coupon_percent').val(response.coupon_percent); $('#shipping_charges').val(response.shipping_charge); if(response.shippingamount!=0){ $("#offershipping").html("Free Shipping Available In Order Total Above <i class='fa fa-rupee'></i> "+response.shippingamount); } if (response.total < 900) { if(response.cod_charges>0){ $('#cod_charge').html(response.cod_charges); } else{ $('#cod_charge').html(0); } } else{ $('#cod_charge').html(0); } $('.shipping_charge_response').html(response.shipping_charge); $('.grand_total_with_tax_response').html(response.grand_total_with_tax); $('.cartGrandTotal').html(response.grand_total_with_tax); $('#grand_total').val(response.grand_total_with_tax); $('#cartToalReview').attr('cart_total',response.total) } }); }
	...
	$(document).on('click','.wishList', function () { var prd_id=$(this).attr('prd_id'); if(window.user_login==1){ var token="{{ csrf_token() }}"; $.ajax({ type:'POST', async:false, url:"{{ route('add_to_wishlist') }}", data:{ "prd_id":prd_id, "_token":token }, success:function(data) { 
        var myObj = JSON.parse(data); update_wishlist();
             ...
             $(".wishlistModalResponse").html("Product Added To Your Wishlist");
                ...
                $('.prdheart'+prd_id).addClass('text-danger');
                 $('#wishlistModal').modal('show');
                 $(".wishlistModalResponse").html("Product Updated To Your Wishlist"); 
