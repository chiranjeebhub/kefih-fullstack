...
<div class="">
        ...
        <a href="{{ route('add_description',(base64_encode($prd_id))) }}"
					...
					<tbody>
					  ...
					  @foreach($description_data as $description)
						...
						<tr>
							...
							<td>{{$description->product_descrip_title}}</td>
							<td>{{$description->product_descrip_content}}</td>
							<td>
            <a href="{{route('edit_description', [base64_encode($prd_id),base64_encode($description->id)])}}"
            onclick = "if (! confirm('Do you want to edit ?')) { return false; }"
            >
                	<i class="fa fa-pencil text-blue" aria-hidden="true"></i>
            </a>&nbsp;|&nbsp;
            <a href="{{route('delete_description', base64_encode($description->id))}}"
