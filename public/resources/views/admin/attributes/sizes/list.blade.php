		...
		<div class="row searchmain-row">
			<div class="col-sm-2  col-md-5">
                        ...
                        <button type="button" class="btn btn-danger btnSubmitTrigger commonClassDisableButton" disabled>Bulk Delete</button>
                        					...
                        					<li>
                        <form role="form" action="{{route('bulkActiveSize')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
				  ...
				  <table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
					  ...
					  @foreach($Sizes as $size)
						...
						<tr>
							<td><input type="checkbox" class="checkbox multiple_select_checkBox checkedProduct" name="product_id[]" value="{{$size->id}}"></td>
							<td>{{$size->id}}</td>
							<td>{{$size->name}}</td>
							<td>
